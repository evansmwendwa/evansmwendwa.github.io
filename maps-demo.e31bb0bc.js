...
(function () {
  ...
  routes.forEach(function (_ref, index) {
    ...
    li.onclick = function () {
      ...
      li.append(debugElem);
      calculateAndDisplayRoute(directionsService, directionsRenderer, routes[index].shops);
    };
  ...
  var debugElem = document.createElement("div");
  debugElem.id = "debug";
  firstElem.append(debugElem);

  var getWaypoints = function getWaypoints(index) {
    return routes[index].shops;
  };

  var initMap = function initMap() {
    directionsRenderer.setMap(map);
    calculateAndDisplayRoute(directionsService, directionsRenderer, routes[2].shops);
  };

  var calculateAndDisplayRoute = function calculateAndDisplayRoute(directionsService, directionsRenderer, data) {
    var firstChunk = data.slice(0, 25);
    var first = firstChunk.shift();
    var last = firstChunk.pop();
    var waypts = firstChunk.map(function (_ref2) {
    ...
    directionsService.route({
      origin: {
        lat: first.location[0],
        lng: first.location[1]
      },
      destination: {
        lat: last.location[0],
        lng: last.location[1]
      ...
      if (status === 'OK') {
        ...
        var _debugElem = document.querySelector("#debug");

        if (_debugElem) {
          _debugElem.innerHTML = "<pre>".concat(JSON.stringify(response.routes[0].legs), "</pre>");
        }
...
//# sourceMappingURL=/maps-demo.e31bb0bc.js.map